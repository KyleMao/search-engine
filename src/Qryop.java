/**
 * All query operators are subclasses of the Qryop class. Most of this class is abstract, because
 * different types of query operators (inverted list, score list) have different subclasses, and
 * each query operator has its own subclass. This class defines the common interface to query
 * operators, and is a place to store data structures and methods that are common to all query
 * operators.
 *
...
public abstract class Qryop {
  ...
  /**
   * Removes an argument from the list of query operator arguments. This simplifies the design of
   * some query parsing architectures.
   * 
